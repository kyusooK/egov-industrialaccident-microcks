<template>
    <v-card outlined>
        <v-card-title>
            Assessment # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="AssessorId" v-model="item.assessorId" :editMode="editMode" @change="change" />
            </div>
            <div>
            ...
            <div>
                <String label="Comments" v-model="item.comments" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/assessments/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
