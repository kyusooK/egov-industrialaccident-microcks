...
public class EgovBBSAttributeManageServiceImpl
    ...
    public BoardMasterVO selectBBSMasterInf(BoardMaster searchVO)
        ...
        if (flag != null && flag.trim().equalsIgnoreCase("true")) {
            BoardMasterVO options = addedOptionsDAO.selectAddedOptionsInf(
                searchVO
            );
    ...
    public Map<String, Object> selectNotUsedBdMstrList(BoardMasterVO searchVO)
        throws Exception {
        List<BoardMasterVO> result = attrbMngDAO.selectNotUsedBdMstrList(
            searchVO
        );
