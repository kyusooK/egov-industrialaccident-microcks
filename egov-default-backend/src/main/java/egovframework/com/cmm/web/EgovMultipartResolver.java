...
public class EgovMultipartResolver extends CommonsMultipartResolver {
     ...
     */
    @Override
    protected MultipartParsingResult parseFileItems(
        ...
        String whiteListFileUploadExtensions = EgovProperties.getProperty(
        ...
        for (Iterator<FileItem> it = fileItems.iterator(); it.hasNext();) {
            ...
            if (fileItem.isFormField()) {
                ...
                String[] curParam = multipartParameters.get(
                    fileItem.getFieldName()
                );
                if (curParam == null) {
                    // simple form field
                    multipartParameters.put(
                        fileItem.getFieldName(),
                        new String[] { value }
                    );
                } else {
                ...
                String fileExtension = EgovFileUploadUtil.getFileExtension(
                    fileName
                );
                log.debug("Found File Extension = " + fileExtension);
                if (
                    whiteListFileUploadExtensions == null ||
                    "".equals(whiteListFileUploadExtensions)
                ) {
                    ...
                    if (fileName == null || "".equals(fileName)) {
                        ...
                        if ("".equals(fileExtension)) { // 확장자 없는 경우 처리 불가
                            throw new SecurityException(
                                "[No file extension] File extension not allowed."
                            );
                        }
                        if (
                            (whiteListFileUploadExtensions + ".").contains(
